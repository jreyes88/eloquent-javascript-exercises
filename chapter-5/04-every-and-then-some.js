/*
  Every and Then Some
  Arrays also come with the standard methods `every` and `some`. Both take a predicate function that, when called with an array element as argument, returns true or false. Just like `&&` returns a true value only when the expressions on both sides are true, every returns true only when the predicate returns true for all elements of the array. Similaraly, some returns true as soon as the predicate rerturns true for any of the elements. They do not process more elements than necessary -- for example, if some finds that the predicate holds for the first element of the array, it will not look at the values after that.
...
// My Solution (tests taken from sandbox):
